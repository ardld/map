...
// Behavior:
//   • Total height fixed to 600px
//   • Only the left list scrolls
//   • Clicking a marker or a list item updates the left Contents panel
//     (NO InfoWindow on the map)
...
/* HTML template — fixed 600px height; only the left list scrolls; map centered on Romania
   IMPORTANT CHANGE: No InfoWindow. Marker/list click updates left Contents only. */
...
<div id="wrap">
  <div id="left">
    ...
    <div id="explain">
      <p>Select a place from the list or a pin on the map. When you pick one, the photo appears here. Click the photo for full-screen view.</p>
      <h3>&nbsp;</h3>
    </div>
...
function buildTOC(groups) {
  ...
  groups.forEach((g, idx)=>{
    ...
    item.onclick = ()=>{
      ...
      renderExplain(g, 0); // ← update Contents (no InfoWindow)
    };
...
window.initMap = async function initMap() {
  ...
  try {
    ...
    window.groups = groups = nonEmpty;
    markers = [];

    // plain markers (no cluster); on click -> update Contents only
    groups.forEach((g, idx) => {
      const m = new google.maps.Marker({ position: { lat: g.lat, lng: g.lng }, map });
      m.addListener('click', () => {
        map.panTo({ lat: g.lat, lng: g.lng });
        map.setZoom(Math.max(map.getZoom(), 10));
        // Update Contents panel, do NOT show InfoWindow
        renderExplain(g, 0);
        // also highlight the selected item in the list
